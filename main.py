...
# --- Configuration ---
# Adjust these column names based on your actual CSV files
COUNTRY_COL = 'country'
YEAR_COL = 'time_period'
VALUE_COL_IND1 = 'obs_value'     # Value column in Indicator 1 file
VALUE_COL_IND2 = 'obs_value'     # Value column in Indicator 2 file
INDICATOR_NAME_COL = 'indicator' # Column containing the indicator's descriptive name

# Metadata columns (needed for choropleth map)
METADATA_FILE = 'UNICEF Metadata.csv'
METADATA_COUNTRY_COL = 'country' # Column in Metadata to match COUNTRY_COL in indicators
ISO_CODE_COL = 'alpha_3_code'    # Column in Metadata with ISO alpha-3 country codes

# Settings for Bar Chart (Plot 1)
SORT_BARS_ASCENDING = True # True if lower Indicator 1 values are better (e.g., mortality)
LIMIT_BAR_CHART_COUNTRIES = 50 # Limit countries shown for readability (None to show all)
# --- End Configuration ---
...
def load_and_prepare_data(indicator1_path, indicator2_path, metadata_path):
    ...
    try:
        ...
        try:
            ...
            print(f"Metadata file '{metadata_path}' loaded successfully.")
        ...
        if not df_ind1_clean.empty and not df_ind2_clean.empty:
            ...
            if not df_scatter_agg.empty: print("Aggregated scatter plot data prepared.")
            else: print("Warning: No overlapping countries with data for scatter plot.")
        else:
        ...
        if df_meta is not None and latest_year is not None and not df_ind1_clean.empty:
            ...
            if not df_ind1_latest.empty:
                ...
                if not df_map_data.empty:
                    print(f"Map data prepared for {len(df_map_data)} countries for year {latest_year}.")
                else:
                    print(f"Warning: No matching countries found between Indicator 1 data (Year {latest_year}) and Metadata based on '{COUNTRY_COL}'/'{METADATA_COUNTRY_COL}'. Cannot generate map.")
            else:
...
def create_choropleth_map(df_map, iso_code_col, color_col, hover_col, ind_name, year):
    ...
    print(f"Generating Plotly Choropleth map for {ind_name} ({year})...")
...
if __name__ == "__main__":
    ...
    print("\n--- Generating Plots ---")
    print("Plots will open in separate windows/tabs.")
